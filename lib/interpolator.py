...
class Interpolator(nn.Module):
    ...
    def getMaxMinXY(self, B, N, H, W):
        ...
        if B1 == B and N1 == N and B2 == B and N2 == N:
            ...
            self.maxXY = self.maxXY.cuda()
            self.minXY = self.minXY.cuda()
    ...
    def forward(self, feature, keypoints):
        """
        ...
        iX = (
            iX.unsqueeze(2).expand(-1, -1, 2, -1).long()
        )  # B x 32 x 2 x 2 ( x0 x1; x0 x1 )
...
class LocationInterpolator(nn.Module):
    ...
    def forward(self, ijB_A, keypoints):
        ...
        """
        xyB_A = (
            self.interpolator(ijB_A.float(), keypoints) * self.interpolator.im_fe_ratio
        )
        return xyB_A.transpose(2, 1)


class InverInterpolator(Interpolator):
    def __init__(self, im_fe_ratio, kernel_size=5, N=32, mode=1):
        ...
        if kernel_size > 0:
            ...
            self.gaussian_filter = gaussian_filter.cuda()
    ...
    def get_1nn(self, Xg, keypoint_g, H, W):
        ...
        )  # B x N x 2 float gt coordinate in feature map
        maxXY, minXY = self.getMaxMinXY(B, N, H, W)

        boundedXY = torch.max(xyGt, minXY)  # B x N x 2, index of X, Y in feature map
        boundedXY = torch.min(
            boundedXY, maxXY
        )  # B x N x 2, index of X, Y in feature map

        boundedXY = boundedXY.long()
        indices = boundedXY[:, :, 1] * W + boundedXY[:, :, 0]  # B x N x 1
        indices = indices.unsqueeze(2)
        coeff = torch.ones(B, N, 1).cuda()
        onehot = torch.zeros(B, N, H * W).cuda()
        onehot.scatter_(dim=2, index=indices, src=coeff)
        if self.kernel_size > 0:
            onehot = self.gaussian_filter(onehot.view(B, N, H, W)).view(
                B, N, H * W
            )  # add gaussian blur
