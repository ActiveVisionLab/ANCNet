...
def pck_metric(batch, batch_start_idx, matches, stats, alpha=0.1, use_cuda=True):

    source_im_size = batch["source_im_size"]
    target_im_size = batch["target_im_size"]

    source_points = batch["source_points"]  # w.r.t. the original image coordinate #224
    target_points = batch["target_points"]  # B x 2 x N

    # warp points with estimated transformations
    target_points_norm = PointsToUnitCoords(
        target_points, target_im_size
    )  # convert from image coordinate to -1, 1
